...
func PutDraftHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.PutDraftReq
		if err := httpx.Parse(r, &req); err != nil {
			httpx.Error(w, err)
			return
		}

		l := prompt.NewPutDraftLogic(r.Context(), svcCtx)
		resp, err := l.PutDraft(&req)
