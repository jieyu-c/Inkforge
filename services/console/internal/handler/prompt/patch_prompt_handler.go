...
func PatchPromptHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.PatchPromptReq
		if err := httpx.Parse(r, &req); err != nil {
			httpx.Error(w, err)
			return
		}

		l := prompt.NewPatchPromptLogic(r.Context(), svcCtx)
		resp, err := l.PatchPrompt(&req)
